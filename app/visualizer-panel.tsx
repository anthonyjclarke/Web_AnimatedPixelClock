...
export function VisualizerPanel({clock}:{clock:Options}){
  ...
  return <>
    ...
    <div className="audio-layout">
      ...
      <section className="audio-card"><h2>Visualizer effect</h2><div className="effect-options">{effects.filter(effect=>effect.available).map(effect=><button className={'style-card '+(o.effect===effect.id?'selected':'')} key={effect.id} aria-pressed={o.effect===effect.id} onClick={()=>setO(v=>({...v,effect:effect.id}))}>{effect.name}<span className="selected-dot"/></button>)}</div><div className="setting-row"><label htmlFor="viz-clock">Corner clock</label><Switch id="viz-clock" checked={o.showClock} onCheckedChange={showClock=>setO(v=>({...v,showClock}))}/></div><p className="audio-note">Uses your clock’s timezone and time format.</p>{o.effect===15&&<><p className="audio-note">Glyph bars, bass-driven rain and held peaks. Code EQ by <a href="https://github.com/NickoScope">Nikolay Miroshnichenko (NickoScope)</a>, adapted from <a href="https://github.com/NickoScope/AnimatedPixelClock">his AnimatedPixelClock fork</a> · <a href="https://github.com/NickoScope/AnimatedPixelClock/commit/85c9be92a5b33b17c126ab63db6bc2ae7c9cc331">source commit</a>.</p><div className="setting-row"><label htmlFor="code-eq-glitch">Beat glitch</label><Switch id="code-eq-glitch" checked={o.codeEqGlitch!==false} onCheckedChange={codeEqGlitch=>setO(v=>({...v,codeEqGlitch}))}/></div></>}{o.effect===6&&<>
